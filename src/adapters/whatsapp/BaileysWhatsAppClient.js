...
export class BaileysWhatsAppClient extends IWhatsAppClient {
  constructor(options = {}) {
    ...
    this.humanTakeoverManager = options.humanTakeoverManager || null;
    this.messageDebouncer = options.messageDebouncer || null;
    this.conversationMemoryStore = options.conversationMemoryStore || null;
  }

  setHumanTakeoverManager(manager) {
    this.humanTakeoverManager = manager;
  }

  setMessageDebouncer(debouncer) {
    this.messageDebouncer = debouncer;
  }

  setConversationMemoryStore(store) {
    this.conversationMemoryStore = store;
  }
  ...
  async connect() {
    ...
    this.sock.ev.on('messages.upsert', async ({ messages, type }) => {
      ...
      for (const msg of messages) {
        ...
        // 1. Skip messages generated by this bot instance (by ID or self echo)
        const isBotOutbound = Boolean(
          (this.sentMessageIds && this.sentMessageIds.has(msg.key?.id)) ||
          (msg.key?.id?.startsWith('BAE5')) ||
          (isSelfChat && cleanText && this.sentTexts && this.sentTexts.has(cleanText))
        );

        if (isBotOutbound) {
          if (this.sentMessageIds && msg.key?.id) {
            this.sentMessageIds.delete(msg.key.id);
          }
          continue;
        }

        // 2. Human Takeover: Outbound messages sent by the owner personally from their phone
        if (msg.key.fromMe && !isSelfChat) {
          console.log(`\n👤 [Owner Outbound Reply] To: ${remoteJid} | Text: "${cleanText}"`);
          if (this.humanTakeoverManager) {
            this.humanTakeoverManager.recordOwnerReply(remoteJid);
          }
        ...
        // 3. Drop any empty message (receipts, reactions, status updates, sync events)
        if (!isAudio && !cleanText) {
          continue;
        }
